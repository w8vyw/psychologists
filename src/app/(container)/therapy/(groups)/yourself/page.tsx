	...
	return (
		<>
			...
			<Text className={styles.text}>
				...
				<p>
					Специально для Вас открыта психотерапевтическая группа «Обретая себя», где вы сможете с
					помощью группы людей, испытывающих такие же проблемы, под руководством квалифицированного
					психолога в безопасной, конфиденциальной и доверительной атмосфере разобраться в себе,
					решить свои трудности, выйти на новый уровень жизни без употребления химических веществ,
					так как вам это станет не нужно и не интересно. Психотерапевтическая группа – это то
					место, где каждый получает понимание, поддержку и заботу! Также, благодаря проработке
					эмоциональной сферы, вы откроете то изобилие чувств, которое в вас есть, но употреблением
					хим. веществ они были подавлены и заморожены, жизнь заиграет новыми потрясающими и
					здоровыми красками, а не тем однообразием и серостью, которое есть в жизни каждого
					зависимого.
				</p>
			</Text>
			<List
			...
			/>
			<List
				heading='Программа "Свобода от зависимости". Что входит:'
				items={{
					column1: [
						'4 индивидуальных сессии.',
						'4 психотерапевтические группы.',
						'4 тематических лекции.',
						'2 занятия по тематической литературе.',
						'Знакомство и посещение групп самопомощи.',
						'Трансформационная игра.'
					],
					column2: [
						'Выезд в интересное место (театр, музей, выставка и т.д.)',
						'1 семейная сессия по запросу.',
						'1 сессия для близкого родственника по запросу.',
						'Ежедневно - утренние медитации.',
						'Ежедневно - онлайн встречи: «Итоги дня».',
						'Ежедневно-поддержка (общение в мессенджерах). '
					]
				}}
			/>
			<Text>
				<p>Стоимость - 79900 рублей.</p>
			</Text>
